    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            if self.path in ("/", "/index.html"):
                ...
                self.end_headers()
                seq = buffer._sequence
                closed = buffer._closed
                self.wfile.write(
                    f'{{"sequence": {seq}, "closed": {str(closed).lower()}}}\n'.encode()
                )
        ...
        def _stream(self) -> None:
            # Setze einen Timeout fuer blockierende Clients (z.B. hängender Tracker)
            ...
            try:
                ...
                log.debug("client disconnected or timed out")
