...
| **Nächster Schritt** | Release-Stand: M1 Recht erledigt (NOTICE, Schwärzung, Guard+Tests), M3 Packaging **komplett erprobt** (24.08. spät): deb-Build auf dem Pi (2 Fixes: dh `--with python3`, explizites `.install` weil pybuild das Wheel in tmp liegenlässt), `python3-skyshutter_0.2.0_all.deb` + `skyshutter-pi_0.2.0_all.deb` gebaut, Install-Smoke bestanden (systemweiter Import, `/usr/bin/skyshutter`, Beispiel-Units), purgiert. Pi-Deploy via git-bundle auf `cc562ed+` (push weiterhin regelblockiert). **Offen:** GitHub-push + R2 History-Bereinigung (replace-text liegt bereit) + Tag v0.2.0; danach Hardware-Batterie E1-E3 (2. Session, Zoom read-only, 0x2082-87) + `liveview --duration`. |
