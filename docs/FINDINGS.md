...
| **Nächster Schritt** | Release-Vorbereitung steht (docs/release-plan.md, tplan 10/10): M1 Recht erledigt (NOTICE.md, Rig-Secrets geschwärzt → docs/secrets.local.md lokal, check-secrets-Guard + Tests), M3 Packaging erledigt (v0.2.0, pi-Extra, debian/-Gerüst mit User-Unit-Beispielen, CI, Wheel-Smoke OK). **Offen:** push (Regel) + R2 History-Bereinigung (filter-repo, replace-text liegt in /tmp/opencode) + Pi-Rebind + Hardware-Sanity nach S. Danach Fernsteuer-Liste durchmessen (Zoom 0x9016, read-only-Batterie E1-E3, siehe Deep-Dive) und CV-Pipeline. |
